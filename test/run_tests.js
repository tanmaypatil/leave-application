...
describe('graphql sanity', function() {
    describe('#emp_master()', function() {
      it('should return emp master when queried against it', function() {
        let query_name = test_cases[0].request
        let query_str = util.getGraphQLQueryStr(query_name)
        let response = util.executeGraphQLQuery(query_str,null)
        console.log('inside graphql sanity ' + response)
